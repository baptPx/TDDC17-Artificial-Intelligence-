...
public class QLearningController extends Controller {
	...
	void performAction(int action) {
		...
		if(action == 0) resetRockets();
		else if(action == 1) moveForward();
		else if(action == 2) moveRight();
		else if(action == 3) moveRightSlow();
		else if(action == 4) moveLeft();
		else moveLeftSlow();
...
//			double current_reward = StateAndReward.getRewardAngle(angle.getValue(), vx.getValue(), vy.getValue());
			...
			double current_reward = StateAndReward.getRewardHover(angle.getValue(), vx.getValue(), vy.getValue());
			...
			if (previous_state != null) {
				...
				Qtable.put(prev_stateaction,
						...
						(current_reward + GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state ) - Qtable.get(prev_stateaction))
				);
...
//					printTable();
			...
			} else System.out.println("new state null");
	...
	public void printTable() {
		for(String i : Qtable.keySet()) {
			System.out.println(i + "    |   " + Qtable.get(i));
		}
	}
